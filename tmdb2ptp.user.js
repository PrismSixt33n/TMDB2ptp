...
window.addEventListener('load', function() {
    ...
    xhr.onload = function() {
        if (xhr.status != 200) {
            ...
            var url2 = 'https://api.themoviedb.org/3/movie/' + movieId + '/recommendations?api_key=' + apiKey;
            console.log("API Call URL: ", url2);
            xhr2.open('GET', url2);
            xhr2.send();
            ...
            xhr2.onload = function() {
                if (xhr2.status != 200) {
                    ...
                    var recommendations = JSON.parse(xhr2.responseText).results;
                    ...
                    if (recommendations.length > 0) {
                        ...
                        mainColumn.insertBefore(panel, mainColumn.children[3]);
                        for (var i = 0; i < 8; i++) {
                            var movie = recommendations[i];
                            var movieDiv = document.createElement('div');
                            movieDiv.style.width = '11%';
                            movieDiv.style.margin = '0.5%';
                            var xhr3 = new XMLHttpRequest();
                            var url3 = 'https://api.themoviedb.org/3/movie/' + movie.id + '?api_key=' + apiKey;
                            xhr3.open('GET', url3, false);
                            xhr3.send();
                            if (xhr3.status == 200) {
                                var movieDetails = JSON.parse(xhr3.responseText);
                                var imdbId = movieDetails.imdb_id;
                                var img = document.createElement('img');
                                img.src = 'https://image.tmdb.org/t/p/w500' + movie.poster_path;
                                img.style.width = '100%';
                                img.title = movie.title + ' - ' + movieDetails.overview;

                                var link = document.createElement('a');
                                link.href = 'https://passthepopcorn.me/torrents.php?searchstr=' + imdbId;
                                link.appendChild(img);

                                movieDiv.appendChild(link);
                                panelBody.appendChild(movieDiv);
                            }
                        }
                    }
                }
